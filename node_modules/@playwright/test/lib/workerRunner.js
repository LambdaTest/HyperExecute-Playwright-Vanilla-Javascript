...
class WorkerRunner extends _events.EventEmitter {
  ...
  _createWorkerTeardownDiagnostics() {
    ...
    const message = [_safe.default.red(`Worker teardown error. This worker ran ${count}${lastMessage}:`), ...this._lastRunningTests.map(testInfo => formatTestTitle(testInfo._test, testInfo.project.name))].join('\n');
    return {
      message
    };
  }
  ...
  async _teardownScopes() {
    ...
    const timeoutManager = new _timeoutManager.TimeoutManager(this._project.config.timeout);
