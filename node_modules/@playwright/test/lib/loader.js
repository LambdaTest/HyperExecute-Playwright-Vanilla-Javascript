...
class Loader {
  ...
  static async deserialize(data) {
    ...
    if ('file' in data.configFile) await loader.loadConfigFile(data.configFile.file);else loader.loadEmptyConfig(data.configFile.configDir);
    return loader;
  }
  ...
  loadEmptyConfig(configDir) {
    this._processConfigObject({}, configDir);

    return {};
  }

  _processConfigObject(config, configDir) {
    ...
    validateConfig(this._configFile || '<default config>', config); // Resolve script hooks relative to the root dir.
    ...
    if (config._screenshotsDir !== undefined) config._screenshotsDir = path.resolve(configDir, config._screenshotsDir);
    if (config.snapshotDir !== undefined) config.snapshotDir = path.resolve(configDir, config.snapshotDir);
    const configUse = (0, _util.mergeObjects)(this._defaultConfig.use, config.use);
    config = (0, _util.mergeObjects)((0, _util.mergeObjects)(this._defaultConfig, config), {
      use: configUse
    });
    ...
    this._fullConfig.reporter = takeFirst(toReporters(this._configOverrides.reporter), resolveReporters(config.reporter, configDir), baseFullConfig.reporter);
    ...
    const projects = 'projects' in config && config.projects !== undefined ? config.projects : [config];

    for (const project of projects) this._addProject(config, project, throwawayArtifactsPath);
  ...
  _addProject(config, projectConfig, throwawayArtifactsPath) {
    ...
    if (projectConfig._screenshotsDir !== undefined) projectConfig._screenshotsDir = path.resolve(this._configDir, projectConfig._screenshotsDir);
    ...
    const screenshotsDir = takeFirst(this._configOverrides._screenshotsDir, projectConfig._screenshotsDir, config._screenshotsDir, path.join(testDir, '__screenshots__', process.platform, name));
    const fullProject = {
      ...
      use: (0, _util.mergeObjects)((0, _util.mergeObjects)(config.use, projectConfig.use), this._configOverrides.use)
    };
  ...
  async _requireOrImport(file) {
    ...
    try {
      ...
      if (error.code === 'ERR_UNKNOWN_FILE_EXTENSION' && error.message.includes('.ts')) {
        throw (0, _util.errorWithFile)(file, `Cannot import a typescript file from an esmodule.\n${'='.repeat(80)}\nMake sure that:
