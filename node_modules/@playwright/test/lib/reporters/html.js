...
class HtmlReporter {
  ...
  async onEnd() {
    ...
    const {
      ok,
      singleTestId
    } = await builder.build(new _raw.default().generateAttachments(this.config), reports);
    ...
    if (shouldOpen) {
      ...
      const relativeReportPath = outputFolder === standaloneDefaultFolder() ? '' : ' ' + _path.default.relative(process.cwd(), outputFolder);
      console.log('');
