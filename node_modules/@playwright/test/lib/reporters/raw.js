...
class RawReporter {
  ...
  generateAttachments(config) {
    return this._createAttachments(config._attachments);
  }
  ...
  generateProjectReport(config, suite) {
    ...
    const report = {
      ...
      suites: suite.suites.map(fileSuite => {
        ...
        const fileId = (0, _utils.calculateSha1)(fileSuite.location.file.split(_path.default.sep).join('/'));
  ...
  _createAttachments(attachments, ioStreams) {
    const out = [];
