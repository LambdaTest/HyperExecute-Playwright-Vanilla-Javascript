...
class Dispatcher {
  constructor(loader, testGroups, reporter) {
    ...
    for (const group of testGroups) {
      ...
      for (const test of group.tests) this._testById.set(test._id, {
        test,
  ...
  async _runJob(worker, testGroup) {
    ...
    const onDone = params => {
      ...
      const massSkipTestsFromRemaining = (testIds, errors) => {
        remaining = remaining.filter(test => {
          if (!testIds.has(test._id)) return true;
      ...
      if (remaining.length) {
        ...
        this._queuedOrRunningHashCount.set(testGroup.workerHash, this._queuedOrRunningHashCount.get(testGroup.workerHash) + 1); // Perhaps we can immediately start the new job if there is a worker available?
