...
class SnapshotHelper {
  constructor(testInfo, snapshotPathResolver, anonymousSnapshotExtension, configOptions, nameOrOptions, optOptions) {
    ...
    this.mimeType = (_mime$getType = mime.getType(_path.default.basename(this.snapshotPath))) !== null && _mime$getType !== void 0 ? _mime$getType : 'application/octet-string';
  ...
  handleDifferent(actual, expected, previous, diff, diffError, log, title = `${this.kind} comparison failed:`) {
    ...
    if (previous !== undefined) {
      ...
      output.push(`Previous: ${_safe.default.yellow(this.previousPath)}`);
    }
...
function toMatchSnapshot(received, nameOrOptions = {}, optOptions = {}) {
  ...
  const helper = new SnapshotHelper(testInfo, testInfo.snapshotPath.bind(testInfo), determineFileExtension(received), ((_testInfo$project$exp = testInfo.project.expect) === null || _testInfo$project$exp === void 0 ? void 0 : _testInfo$project$exp.toMatchSnapshot) || {}, nameOrOptions, optOptions);
...
async function toHaveScreenshot(pageOrLocator, nameOrOptions = {}, optOptions = {}) {
  var _testInfo$project$exp2, _config$animations, _config$fonts, _config$scale, _config$caret;

  const testInfo = (0, _globals.currentTestInfo)();
  if (!testInfo) throw new Error(`toHaveScreenshot() must be called during the test`);
  const config = (_testInfo$project$exp2 = testInfo.project.expect) === null || _testInfo$project$exp2 === void 0 ? void 0 : _testInfo$project$exp2._toHaveScreenshot;
  const helper = new SnapshotHelper(testInfo, testInfo._screenshotPath.bind(testInfo), 'png', {
  ...
  const screenshotOptions = {
    ...
    _fonts: (_config$fonts = config === null || config === void 0 ? void 0 : config.fonts) !== null && _config$fonts !== void 0 ? _config$fonts : 'ready',
  ...
  if (helper.updateSnapshots === 'all' || !hasSnapshot) {
    ...
    if (errorMessage) {
      const title = actual && previous ? `Timeout ${timeout}ms exceeded while generating screenshot because ${locator ? 'element' : 'page'} kept changing` : `Timeout ${timeout}ms exceeded while generating screenshot`;
    ...
    if (!hasSnapshot) return helper.handleMissing(actual);
  ...
  return errorMessage ? helper.handleDifferent(actual, expected, undefined, diff, errorMessage, log) : helper.handleMatching();
}
