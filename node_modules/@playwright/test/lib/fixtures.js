...
class Fixture {
  ...
  async teardown(timeoutManager) {
    if (!this._teardownWithDepsComplete) this._teardownWithDepsComplete = this._teardownInternal(timeoutManager);
  ...
  async _teardownInternal(timeoutManager) {
    ...
    try {
      for (const fixture of this.usages) await fixture.teardown(timeoutManager);
...
class FixtureRunner {
  ...
  async resolveParametersForFunction(fn, testInfo) {
    ...
    for (const registration of this.pool.registrations.values()) {
      ...
      if (registration.auto && !shouldSkip) await this.setupFixtureForRegistration(registration, testInfo);
  ...
  async resolveParametersAndRunFunction(fn, testInfo) {
